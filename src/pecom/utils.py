...
async def clean_address_with_dadata(full_address: str, session: SessionDep) -> str:
    ...
    stmt = select(DadataCache).where(DadataCache.original_address == full_address)
    ...
    # Если в кэше нет, делаем запрос к DaData
    headers = {
...
async def calculate_pecom_delivery(
    ...
    from_city = await clean_address_with_dadata(from_city_name, session)
    to_city = await clean_address_with_dadata(to_city_name, session)
